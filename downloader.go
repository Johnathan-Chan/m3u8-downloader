...
type M3u8Downloader interface {
	// Download ParseM3u8File 解析.m3u8文件
...
// SetIfMerge 是否显示进度条
func (md *m3u8downloader) SetIfMerge(ifMerge bool) {
	md.config.ifMerge = ifMerge
}
...
func (md *m3u8downloader)printInfo(){
	fmt.Printf("下载任务总数：%d\n",md.config.TotalNum)
	fmt.Printf("下载任务线程数：%d\n",md.config.NumOfThreads)
	fmt.Printf("保存目录：%s\n",*(*string)(unsafe.Pointer(&md.config.SaveDirectory)))
	fmt.Printf("保存名称：%s\n",*(*string)(unsafe.Pointer(&md.config.VideoName)))
	fmt.Printf("下载链接：%s\n",*(*string)(unsafe.Pointer(&md.config.Url)))
}
...
func (md *m3u8downloader) Download() error {
	...
	if md.config.ifMerge{
		err = md.MergeFile()
		if err != nil {
			fmt.Println("[MergeError]",err.Error())
			return err
		}
	}
	return nil
...
func (md *m3u8downloader) SaveAsTsFileAndMergeEncryption(index,threadId int,body []byte){
	md.buffer[threadId].Write(md.config.SaveDirectory)
...
func (md *m3u8downloader) MergeFile() error{
	...
	err := mergeFile(*(*string)(unsafe.Pointer(&md.config.SaveDirectory)),
		md.suffixList, *(*string)(unsafe.Pointer(&md.config.VideoName)))
	if err != nil {
		return err
	}
	return nil
